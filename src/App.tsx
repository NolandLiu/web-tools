...
function App() {
  ...
  useEffect(() => {
    applyRouteMetadata(getRouteMetadata(route));
  ...
  const navigate = (next: Page) => {
    if (next === "home") {
      goTo({ kind: "home", lang });
    } else if (next === "not-found") {
      goTo({ kind: "not-found", lang });
    } else {
      goTo({ kind: "info", lang, page: next });
    }
  };
  ...
  const openCategory = (categoryId: CategoryId) => {
    goTo({ kind: "category", lang, categoryId });
  };

  const changeLanguage = (nextLang: Lang) => {
    goTo(switchRouteLanguage(route, nextLang));
  };

  return (
    <AppShell lang={lang} activeTool={activeTool} onLanguageChange={changeLanguage} onNavigate={navigate} onOpenTool={openTool}>
      {route.kind === "home" && (
        <HomeDirectory lang={lang} query={query} setQuery={setQuery} tools={filtered} onOpenTool={openTool} onOpenCategory={openCategory} />
      )}
      {route.kind === "tool" && activeTool && (
        <Suspense fallback={<p className="empty">{messages[lang].open}…</p>}>
          <ToolWorkspace key={activeTool.id} tool={activeTool} lang={lang} onOpenTool={openTool} />
        </Suspense>
      )}
      {route.kind === "category" && (
        <CategoryPage categoryId={route.categoryId} lang={lang} onOpenTool={openTool} />
      )}
      {route.kind === "info" && <InfoPage page={route.page} lang={lang} navigate={navigate} />}
      {route.kind === "not-found" && <InfoPage page="not-found" lang={lang} navigate={navigate} />}
    </AppShell>
  );
}

function HomeDirectory({ lang, query, setQuery, tools, onOpenTool, onOpenCategory }: {
  lang: Lang; query: string; setQuery: (value: string) => void; tools: Tool[]; onOpenTool: (tool: Tool) => void; onOpenCategory: (categoryId: CategoryId) => void;
}) {
  const t = messages[lang];
  return <><section className="hero"><span className="hero-pill"><Icon name="check" size={14} />{t.privacyBadge}</span><h1>{t.siteName}</h1><p>{t.tagline}</p><label className="search-box"><Icon name="search" size={20} /><span className="sr-only">{t.search}</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder={t.search} /></label></section><section className="directory"><div className="section-head"><div><span className="eyebrow">{t.popular}</span><h2>{t.allTools}</h2></div><span>{tools.length} {t.tools}</span></div><div className="category-row" aria-label={t.categories}><button type="button" className="active" onClick={() => setQuery("")}>{t.allTools}</button>{CATEGORY_ORDER.map(id => <button type="button" key={id} onClick={() => onOpenCategory(id)}>{categories[id][lang]}</button>)}</div><div className="tools-grid">{tools.map(tool => <ToolCard key={tool.id} tool={tool} lang={lang} onClick={() => onOpenTool(tool)} />)}</div>{tools.length === 0 && <p className="empty">{t.noResults}</p>}</section></>;
}
  ...
  const pages: Record<Exclude<Page, "home">, Record<Lang, { heading: string; intro: string[]; sections: InfoSection[] }>> = {
    ...
    privacy: {
      en: {
        ...
        intro: ["Last updated: July 2026", "At GoDeskHub, accessible from https://tools.godeskhub.com, the privacy of our visitors is one of our top priorities. This Privacy Policy explains what information may be collected and how we use it."],
        sections: [
          { heading: "1. Information We Collect", bullets: ["Non-Personal Data: like most websites, hosting or analytics systems may collect log-style data such as browser type, date and time, referring pages, and general technical information. This data is not linked to personally identifiable information by GoDeskHub.", "Google Analytics: GoDeskHub uses the Google tag and Google Analytics to understand page visits. Google Analytics may process visit information such as browser and device details, visit time, referrer, and page path.", "Cookies and Web Beacons: GoDeskHub may use cookies to store visitor preferences and improve user experience based on browser settings or language choices."] },
          ...
          { heading: "3. Data Processing", paragraphs: ["Most of our tools execute directly in your web browser. Tool inputs, outputs, passwords, amounts, QR Code content, files, and free-form text are not stored on our server. Tool inputs, outputs, passwords, amounts, QR Code content, files, and free-form text are not sent to Google Analytics by the application."] },
      ...
      "zh-CN": {
        heading: "隐私政策",
        intro: ["最后更新：2026 年 7 月", "GoDeskHub 可通过 https://tools.godeskhub.com 访问。访问者隐私是我们的重要优先事项。本隐私政策说明可能收集的信息类型以及使用方式。"],
        sections: [
          { heading: "1. 我们收集的信息", bullets: ["非个人数据：与多数网站类似，托管或分析系统可能收集浏览器类型、日期时间、来源页面和一般技术信息等日志类数据。GoDeskHub 不会将这些数据与个人身份信息关联。", "Google Analytics：GoDeskHub 使用 Google tag 和 Google Analytics 了解页面访问情况。Google Analytics 可能处理浏览器和设备信息、访问时间、来源页面和页面路径等访问类信息。", "Cookie 和 Web Beacons：GoDeskHub 可能使用 cookie 保存访问偏好，并根据浏览器设置或语言选择优化体验。"] },
          ...
          { heading: "3. 数据处理", paragraphs: ["大多数工具直接在你的浏览器中运行。工具输入、输出、密码、金额、二维码内容、文件和自由文本不会由应用保存到服务器。工具输入、输出、密码、金额、二维码内容、文件和自由文本不会由应用发送到 Google Analytics。"] },
          { heading: "4. 同意", paragraphs: ["使用本网站即表示你同意本隐私政策及其条款。如有隐私问题，请联系 support@godeskhub.com。"] },
        ],
      },
      "zh-TW": {
        heading: "隱私權政策",
        intro: ["最後更新：2026 年 7 月", "GoDeskHub 可透過 https://tools.godeskhub.com 存取。訪客隱私是我們的重要優先事項。本隱私權政策說明可能收集的資訊類型以及使用方式。"],
        sections: [
          { heading: "1. 我們收集的資訊", bullets: ["非個人資料：與多數網站類似，託管或分析系統可能收集瀏覽器類型、日期時間、來源頁面和一般技術資訊等紀錄資料。GoDeskHub 不會將這些資料與個人身分資訊連結。", "Google Analytics：GoDeskHub 使用 Google tag 和 Google Analytics 了解頁面造訪情況。Google Analytics 可能處理瀏覽器和裝置資訊、造訪時間、來源頁面和頁面路徑等造訪類資訊。", "Cookie 和 Web Beacons：GoDeskHub 可能使用 cookie 儲存造訪偏好，並依據瀏覽器設定或語言選擇改善體驗。"] },
          ...
          { heading: "3. 資料處理", paragraphs: ["大多數工具直接在你的瀏覽器中執行。工具輸入、輸出、密碼、金額、QR Code 內容、檔案和自由文字不會由應用程式儲存到伺服器。工具輸入、輸出、密碼、金額、QR Code 內容、檔案和自由文字不會由應用程式傳送到 Google Analytics。"] },
    ...
    terms: {
      en: {
        ...
        sections: [
          { heading: "1. Acceptance of Terms", paragraphs: ["By accessing or using GoDeskHub (https://tools.godeskhub.com), you agree to be bound by these Terms of Service and all applicable laws and regulations. If you do not agree with any of these terms, you must not use this site."] },
      ...
      "zh-CN": {
        ...
        sections: [
          { heading: "1. 接受条款", paragraphs: ["访问或使用 GoDeskHub（https://tools.godeskhub.com）即表示你同意受本服务条款以及适用法律法规约束。如不同意任何条款，请不要使用本站。"] },
      ...
      "zh-TW": {
        ...
        sections: [
          { heading: "1. 接受條款", paragraphs: ["存取或使用 GoDeskHub（https://tools.godeskhub.com）即表示你同意受本服務條款以及適用法律法規約束。如不同意任何條款，請不要使用本站。"] },
